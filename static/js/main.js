(function(){
	...
	if(embedForm){
		...
		embedForm.addEventListener('submit', async function(e){
			...
			try{
				...
				// Store download URL for button click
				window.downloadUrl = data.download_url;
				window.downloadFilename = data.filename;
				
				// Set direct download link
				qs('#direct-download').href = data.download_url;
	...
	// SIMPLE WINDOWS-COMPATIBLE DOWNLOAD
	...
	if(downloadBtn){
		downloadBtn.addEventListener('click', function(){
			if(window.downloadUrl){
				...
				setTimeout(function(){
					toast('If download didn\'t start, right-click the link and "Save as"', 'warning');
				}, 2000);
			} else {
